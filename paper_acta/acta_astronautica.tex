...
Figure~\ref{fig:3d_cost} shows the MPC stage cost
$J_k = (\mathbf{x}_k - \mathbf{x}_{\mathrm{ref}})^\top \mat{Q}\,
(\mathbf{x}_k - \mathbf{x}_{\mathrm{ref}}) +
\mathbf{u}_k^\top \mat{R}_u\,\mathbf{u}_k$ over the 400-step approach.
The cost peaks during the initial transient when the chaser is far from
the docking axis, then decays over three orders of magnitude as the
trajectory converges.  The oscillatory structure reflects the polhode
period of the tumbling body frame: each time the body frame rotates
through a large-precession phase ($\sim$120~s period), the MPC must
expend additional effort to track the moving docking axis, producing
local cost peaks that are absent in the planar (single-axis) case.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\columnwidth]{fig_cost_history.png}
\caption{MPC stage cost history for the 3D tumbling approach.
  The cost decays from $\sim$10$^4$ to $\sim$10$^1$ as the chaser converges.
  Oscillatory modulation corresponds to the polhode period of the
  intermediate-axis instability; each precession cycle requires
  increased control effort to maintain LOS cone compliance.}
\label{fig:3d_cost}
\end{figure}
