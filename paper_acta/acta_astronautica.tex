...
\caption{Benchmark at $\omega_t = 4$~deg/s (Faraci scenario).
  ...
  computation time.  The RG-OCP solve time (2658~ms per trajectory)
  is from~\cite{Faraci2025}.}
\label{tab:benchmark}
...
\caption{Safe-start coverage versus the scaling parameter $a_{\max}/\omega_t^2$
  for tumble rates 1--6~deg/s.  Star markers indicate the Faraci benchmark
  operating points ($\omega_t = 4$~deg/s).  The collapse of all curves
  confirms $a_{\max}/\omega_t^2$ as the universal feasibility parameter
  across a wider range than the baseline sweep of Table~\ref{tab:nominal}.}
