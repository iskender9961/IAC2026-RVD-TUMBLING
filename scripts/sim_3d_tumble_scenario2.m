%% SIM_3D_TUMBLE_SCENARIO2  —  MPC approach to a dramatically tumbling target
%
%  Scenario 2: Exploits the intermediate-axis (tennis-racket) theorem to
%  generate visually dramatic tumbling at only ~2 deg/s.  The initial
%  angular velocity is aligned predominantly with the intermediate
%  principal axis (I3=600 kg·m²), which is dynamically unstable under
%  Euler's torque-free equations, causing large-amplitude polhode motion.
%
%  The chaser is rendered as a box whose +y body axis always points
%  toward the target.  Reference-frame arrows are drawn at the tip
%  (docking face) of the chaser.
%
%  Outputs (saved to results/3d_tumble_scenario2/):
...
clear; clc; close all;
fprintf('=== 3D Tumble MPC — Scenario 2 (Intermediate-Axis Instability) ===\n');

if ~exist('quadprog', 'file')
    error('sim_3d_tumble_scenario2:noToolbox', ...
...
out_dir = fullfile(fileparts(mfilename('fullpath')), '..', 'results', '3d_tumble_scenario2');
...
%  I1=500  I3=600  I2=800   →  z-axis is the intermediate axis
I_body = diag([500, 800, 600]);

% --- Target initial angular velocity [rad/s] ---
%  Mostly about z-body (intermediate axis) with small perturbations
%  on x,y to trigger the tennis-racket instability.
%  |omega| ≈ 0.0348 rad/s ≈ 2.0 deg/s
omega0_body = [0.005; 0.004; 0.034];
omega_mag   = norm(omega0_body);

% --- Chaser ---
a_max = 0.20;                   % Max thrust [m/s²]
...
% --- MPC ---
Np = 40;   nx = 6;   nu = 3;
Q     = diag([15, 30, 15, 1, 1, 1]);
QN    = 30 * Q;
Ru    = 1e-2 * eye(nu);
Rdu   = 1e4  * eye(nu);

% --- Reference trajectory ---
y_start = 150;   y_end = 5;   tau_ref = 120;
...
fprintf('  |omega_0| = %.2f deg/s (intermediate-axis instability)\n', rad2deg(omega_mag));
fprintf('  r_sync    = %.1f m\n', r_sync);
...
for k = 1:N_steps
    ...
    log_y_ref(k+1)        = y_end + (y_start-y_end)*exp(-(t_now+dt)/tau_ref);
    ...
    if mod(k,50)==0
        fprintf('  Step %3d/%d  t=%3.0fs  dist=%6.1fm  margin=%6.2f  omega_dir_err=%5.1f°  %s\n',...
            k,N_steps,t_now+dt,norm(r_tb_new),mrg,log_omega_dir(k+1),qp_status);
    end
end
...
draw_los_cone_3d(cone_k, y_min, 160, n_faces, [1 0.5 0], 0.08);
plot3([0 0],[y_min 160],[0 0],'g--','LineWidth',1.5);
...
title(sprintf(['Scenario 2: 3D Trajectory in Target Body Frame\n' ...
    '|\\omega_0| = %.1f deg/s (intermediate-axis), ' ...
    'r_0 = [%.0f,%.0f,%.0f] m'], ...
...
tmp_png = fullfile(tempdir,'s2_gif_tmp.png');
...
for fi = 1:n_frames
    ...
    % LOS cone (wireframe only — avoid transparency in batch)
    draw_los_wireframe(cone_k, y_min, max(log_dist(idx)+20,60), n_faces);

    % Docking axis
    plot3([0 0],[y_min max(log_dist(idx)+20,60)],[0 0],'g--','LineWidth',1.2);
    ...
    xlabel('x_B [m]'); ylabel('y_B [m]'); zlabel('z_B [m]');
    title(sprintf('Body Frame | t=%3.0fs | dist=%.1fm | \\omega=[%.1f,%.1f,%.1f]°/s',...
        t_now, log_dist(idx), rad2deg(log_omega(1,idx)),...
        rad2deg(log_omega(2,idx)), rad2deg(log_omega(3,idx))));
    grid on; axis equal;
    rm = max(170, max(abs(log_r_tb(:,1:idx)),[],'all')*1.2);
...
%% ====================================================================
%  SECTION 6 — GIF: LVLH FRAME (TUMBLING TARGET)
%  ====================================================================
...
for fi = 1:n_frames
    ...
    da = R_lvlh_body * [0 0; y_min 80; 0 0];
    ...
    xlabel('x_{LVLH} [m]'); ylabel('y_{LVLH} [m]'); zlabel('z_{LVLH} [m]');
    title(sprintf('LVLH Frame | t=%3.0fs | dist=%.1fm | Target tumbling (%.1f° axis shift)',...
        t_now, log_dist(idx), log_omega_dir(idx)));
    grid on; axis equal;
    rm = max(170, max(abs(log_r_lvlh(:,1:idx)),[],'all')*1.3);
...
sgtitle(sprintf(['Scenario 2 Summary | |\\omega_0|=%.1f°/s (interm. axis) | ' ...
...
xlabel('Time [s]');
title('Target Angular Velocity & Spin-Axis Precession (Intermediate-Axis Instability)');
...
ylabel('x_B [m]'); title('Relative Position in Target Body Frame (Scenario 2)');
...
ylabel('u_{x,B} [m/s^2]');
title('Control Acceleration in Target Body Frame (Scenario 2)');
grid on; set(gca,'FontSize',11,'XTickLabel',[]);
...
plot(t_vec, log_r_tb(2,:),'g-','LineWidth',1.5); hold on;
plot(t_vec, log_y_ref,'k--','LineWidth',1.2);
ylabel('y_B [m]'); title('Approach Profile — Scenario 2 (3D Tumble)');
legend('Docking axis range','Exponential reference','Location','best');
grid on; set(gca,'FontSize',11,'XTickLabel',[]);
...
title('Polhode: Angular Velocity Trajectory (Torque-Free)');
...
fprintf('  Total files: 2 GIFs + %d static figures (PNG+PDF)\n', 9);
...
% ---- Reference ----
function xr = build_reference(t, dt, Np, ys, ye, tau)
    xr=zeros(6,Np+1);
    for j=0:Np
        xr(2,j+1)=ye+(ys-ye)*exp(-(t+j*dt)/tau);
    end
end
...
function R = chaser_attitude(r_tb)
    % R columns = [x_chs, y_chs, z_chs] in target body frame
...
function draw_oriented_box(pos, dims, R, col, al)
    % Draw a box at pos with orientation R and half-dims
...
function draw_axes_at(pos, R, len)
    % Draw RGB reference frame arrows
...
function draw_los_wireframe(ck, ym, ymax, nf)
    % Lightweight wireframe cone (no transparency — safe for batch)
    th = linspace(0, 2*pi, max(nf,16)+1);
    yv = [ym, ymax*0.33, ymax*0.66, ymax];
    for yi = 1:length(yv)
        r = ck * yv(yi);
        plot3(r*cos(th), yv(yi)*ones(size(th)), r*sin(th), ...
            '-','Color',[1 0.6 0.2],'LineWidth',0.8);
    end
    % Longitudinal lines
    for ti = 1:4:length(th)-1
        plot3([0 ck*ymax*cos(th(ti))], [0 ymax], [0 ck*ymax*sin(th(ti))],...
            '-','Color',[1 0.6 0.2 0.5],'LineWidth',0.5);
    end
end
