...
\graphicspath{{../figures/}{../results/}{../results/elliptic/}{../results/benchmark/}}
...
\subsection{Single-Scenario Closed-Loop Approach}

Before the parametric sweep, we examine a single representative case ($\omega_t = 1$~deg/s, $a_{\max} = 0.10$~m/s$^2$) in detail.  The chaser starts at $[0, 200, 0]^\top$~m in LVLH with zero initial velocity.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_approach_profile.png}
\caption{Approach profile for the baseline scenario ($\omega_t = 1$~deg/s, $a_{\max} = 0.10$~m/s$^2$).  \textbf{Top:} distance along the docking axis ($y_{TB}$) vs.\ the exponential reference.  The chaser tracks the reference closely, with small oscillations during the initial transient caused by CWH cross-coupling.  \textbf{Middle:} radial deviation $\sqrt{x_{TB}^2 + z_{TB}^2}$ from the docking axis.  After a 25~m peak during the initial settling, the deviation converges to near zero.  \textbf{Bottom:} minimum LOS cone margin (positive = inside cone).  The margin remains positive throughout, confirming zero LOS violations.}
\label{fig:approach_profile}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{fig_cost_vs_time.png}
\caption{MPC stage cost $J_k$ versus time for the baseline scenario.  The cost peaks at $\sim$10\,000 during the initial transient when the state deviation from the reference is largest, then decays monotonically as the chaser converges to the hold point.  The secondary bump near $t = 40$~s corresponds to the CWH along-track oscillation visible in Fig.~\ref{fig:approach_profile}.  After $t = 300$~s the cost approaches zero, indicating near-perfect tracking.}
\label{fig:cost_vs_time}
\end{figure}

\subsection{Input-Rate Penalty Sensitivity}

The input-rate penalty $R_{\Delta u}$ controls the trade-off between tracking aggressiveness and control smoothness.  Four values are compared: $R_{\Delta u} \in \{10^5, 10^6, 10^7, 10^8\} \times \mathrm{diag}(1, 0.1, 1)$.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_rdu_positions.png}
\caption{Position components versus time for four $R_{\Delta u}$ values, in target body (left) and LVLH (right) frames.  \textbf{$x$-axis:} all scenarios converge $x_{TB} \to 0$; lower $R_{\Delta u}$ produces faster settling but larger initial overshoot (up to 30~m).  In LVLH, the radial excursion reaches $-130$~m due to CWH drift before returning to origin.  \textbf{$y$-axis:} docking-axis approach profiles are nearly identical across all four cases, showing that the exponential reference dominates the approach rate.  \textbf{$z$-axis:} cross-track motion remains within $\pm$50~m in both frames; the $R_{\Delta u} = 10^8$ case shows slightly larger $J_2$-induced drift at late times due to its inability to command rapid corrections.}
\label{fig:rdu_positions}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_rdu_control_tb.png}
\caption{Control acceleration components in $\mathcal{F}_B$ for four $R_{\Delta u}$ values.  \textbf{$u_{x,TB}$:} the lowest penalty ($R_{\Delta u} = 10^5$, blue) produces the most aggressive corrections, with peak thrust of $0.15$~m/s$^2$ and persistent oscillations during hold; higher penalties progressively smooth the response.  \textbf{$u_{y,TB}$:} all cases show similar magnitude ($\pm 0.05$--$0.10$~m/s$^2$) reflecting the common approach reference, but the $R_{\Delta u} = 10^5$ case exhibits higher-frequency content.  \textbf{$u_{z,TB}$:} the cross-track control effort is negligible ($< 0.2$~m/s$^2$) for all cases because the planar tumble does not excite the $z$-axis; the scale confirms that this is a predominantly 2D problem.}
\label{fig:rdu_control}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{fig_rdu_deltav.png}
\caption{Cumulative $\Delta v$ versus time for the four $R_{\Delta u}$ cases.  Lower input-rate penalties produce higher fuel consumption: $R_{\Delta u} = 10^5$ reaches 19.5~m/s at $t = 600$~s, while $R_{\Delta u} = 10^8$ uses only 14.8~m/s---a 24\% reduction.  The curves diverge primarily during the first 100~s (initial manoeuvre) and again during steady-state hold (due to different control chattering levels).  This quantifies the fuel--smoothness trade-off: aggressive penalties track faster but at higher propellant cost.}
\label{fig:rdu_deltav}
\end{figure}
...
feasibility to tumble-rate knowledge at high $\omega_t$.
...
The present analysis assumes pure spin about the body $z$-axis
($\boldsymbol{\omega}_B = [0, 0, \omega_t]^\top$), reducing the problem to
planar rotation of the LOS cone.  This simplification is justified on three
grounds:

\begin{enumerate}
\item \textbf{Dominant mode.}  Many LEO debris objects and defunct satellites
  exhibit principal-axis spin as the dominant tumble mode, particularly after
  energy dissipation has damped nutation~\cite{Schaub2018}.  For such targets,
  the single-axis model captures the primary feasibility driver.

\item \textbf{Conservative approximation.}  Adding transverse angular rates
  ($\omega_x, \omega_y \neq 0$) would increase the apparent rotation-induced
  drift $\|\boldsymbol{\omega}_B \times \mathbf{r}_B\|$ at every point, thereby
  increasing the erosion~\eqref{eq:erosion} and shrinking the safe-start region.
  The single-axis analysis therefore provides an \emph{upper bound} on the safe
  region for a given $\|\boldsymbol{\omega}_B\|$.

\item \textbf{Decoupling at small transverse rates.}  When the transverse
  components satisfy $\omega_x, \omega_y \ll \omega_z$, their contribution to
  the constraint-slack rate $\dot{s}_i$ is perturbative and can be bounded
  independently, enabling a separable worst-case analysis.
\end{enumerate}

The framework extends naturally to full 3D tumble by replacing the planar
rotation velocity $\mathbf{v}_{\mathrm{rot}} = [\omega_t y_B, -\omega_t x_B,
0]^\top$ with the general cross product $\mathbf{v}_{\mathrm{rot}} =
\boldsymbol{\omega}_B(t) \times \mathbf{r}_B$, where
$\boldsymbol{\omega}_B(t)$ is obtained from Euler's equations under torque-free
precession.  The erosion formula~\eqref{eq:erosion} remains valid with this
generalised velocity; the synchronisation bound generalises to
$r_{\mathrm{sync}} = 2a_{\max}/\|\boldsymbol{\omega}_B\|^2$.  The primary
additional complexity is that the time-varying $\boldsymbol{\omega}_B(t)$
produces a non-periodic constraint geometry, requiring evaluation over the
full simulation horizon rather than a single rotation period.  This extension
...
Future work: Hamilton-Jacobi viability kernels for exact safe-set computation, integration with tube-based MPC for online robust tracking, multi-phase approach strategies combining de-tumbling with corridor approach, 3D tumble with full Euler dynamics, and navigation uncertainty integration.
...
evaluated at $\nu_0$ with $J = 0$.  The inverse transform is:
\begin{equation}
  \mat{T}^{-1}(\nu) =
  \begin{bmatrix}
