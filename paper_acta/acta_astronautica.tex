...
\begin{equation}
  \dot{\mathbf{q}}_{B/I}
  = \tfrac{1}{2}\,\mathbf{q}_{B/I} \otimes [0;\;\boldsymbol{\omega}_B],
\label{eq:quat_prop}
\end{equation}
...
Figure~\ref{fig:3d_omega} presents the target angular velocity components
and spin-axis precession angle.  The intermediate-axis instability causes
$\omega_z$ (the initially dominant component) to exchange energy with
$\omega_x$ and $\omega_y$, producing $120^\circ$ of axis precession over
400~s.  The angular momentum magnitude $|\boldsymbol{\omega}|$ remains
constant (torque-free), confirming correct Euler integration.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\columnwidth]{fig_omega_components.png}
\caption{Target angular velocity components (left axis) and spin-axis
  precession from initial $\boldsymbol{\omega}_0$ (right axis, magenta).
  The intermediate-axis instability causes dramatic energy exchange between
  body axes while conserving $|\boldsymbol{\omega}|$ (dashed black).
  The MPC handles this time-varying rotation without modification.}
\label{fig:3d_omega}
\end{figure}
