...
\title{MPC-Based Rendezvous with Tumbling Target:\\
       Body-Fixed LOS Tetrahedral Corridor Constraints}
\author{Iskender O.B.}
\date{}
\maketitle
...
The key feature is that the line-of-sight (LOS) keep-out corridor
constraint---approximated as a tetrahedral (polyhedral) cone---is
expressed in the \emph{target body frame}, so that it naturally
...
The primary cost driver is input smoothness ($\Delta\bu$ penalty),
with a small absolute input regularisation term.
\end{abstract}
...
($\texttt{RelTol} = 10^{-10}$, $\texttt{AbsTol} = 10^{-12}$).
...
\subsection{Body-Fixed LOS Tetrahedral Corridor Constraint}\label{sec:los}
...
\paragraph{Tetrahedral (polyhedral) approximation.}
To keep the problem as a QP (rather than SOCP), the circular cone is
approximated by $m$ half-planes, forming a \emph{tetrahedral corridor}.
Define
\begin{equation}\label{eq:theta_i}
  \theta_i = \frac{2\pi\,(i-1)}{m}, \qquad i = 1,\ldots,m,
  \qquad k = \tan(\alpha).
\end{equation}
Then the tetrahedral inner approximation of~\eqref{eq:soc} is
...
row of the constraint matrix.  The number of faces $m$ controls the
tightness of the approximation; $m = 8$ (octahedral) is used as the
default.

\paragraph{Key advantage.}
Because the state $\bx$ is already expressed in the target body frame,
the corridor constraint~\eqref{eq:poly_cone} is \emph{time-invariant}
in the MPC formulation.  The corridor ``automatically'' rotates with
the tumbling target; no explicit rotation of the constraint normals is
...
\begin{enumerate}
  ...
  \item \textbf{LOS tetrahedral corridor constraints} ($m \cdot (N+1) + (N+1)$ rows). \\
...
\subsection{Solver Rebuild Strategy}

Because the finite-difference linearisation can produce different
sparsity patterns in $A_d$, $B_d$ across steps (zero entries may become
nonzero and vice versa), the OSQP problem is \emph{rebuilt from scratch}
at each MPC step rather than updated in place.  This avoids sparsity
mismatches in OSQP's internal compressed sparse column (CSC)
representation.

While this approach forgoes the computational benefit of in-place
updates, the problem size is modest (typically $\sim 300$ decision
variables) and the rebuild overhead is negligible compared to the
OSQP solve time.  All matrices ($P$, $A_{\text{qp}}$) are assembled
as sparse MATLAB matrices and converted to CSC by the OSQP interface.
...
$y > y_{\min}$ must satisfy \emph{all} $m$ facets of the tetrahedral
corridor~\eqref{eq:poly_cone}:
\[
...
corridor constraints become so tight that no feasible trajectory
...
% ====================================================================
\section{Expected Cost Profile}\label{sec:cost_profile}
% ====================================================================

The stage cost $J_k$ exhibits a characteristic two-phase profile:

\begin{enumerate}
  \item \textbf{Synchronisation phase} ($t \lesssim 2\tau$):
    the chaser must track the rotating docking axis.  The lateral
    deviations $x_T$, $z_T$ are penalised heavily ($Q_{11} = Q_{33} = 15$),
    driving the chaser onto the $+\hat{\bm{y}}_T$ axis.  During this
    phase the cost may increase as the controller initially struggles to
    lock onto the tumbling axis.

  \item \textbf{Approach phase} ($t \gtrsim 2\tau$):
    once synchronised with the target body frame, the cost decreases
    monotonically as the chaser reduces $y_T$ along the docking axis.
    The delta-u penalty ($R_{\Delta u} = 300\,I_3$) ensures smooth
    thruster commands without chatter.
\end{enumerate}

The tuning is designed so that the cost is \emph{not} sinusoidal---it
should rise during sync, then decay monotonically.  If the cost
oscillates, the $R_{\Delta u}$ weight is too high (preventing the
controller from responding to the tumble) or the $Q$ lateral weights
are too low.
...
\begin{figure}[H]
  ...
  \caption{Three-dimensional approach trajectory in the target body frame.
           The wireframe represents the tetrahedral LOS corridor constraint;
           the chaser path (blue) remains inside the corridor throughout the
           manoeuvre.  Coordinate triads for the target body (solid),
           chaser (dashed), and LVLH (dash-dot) frames are shown.}
  \label{fig:3d}
\end{figure}
